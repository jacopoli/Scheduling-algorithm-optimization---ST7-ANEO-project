...
### 3. Upload your input information

```bash
aws s3 cp task_graph.json s3://central-supelec-data-group4/input_data/task_graph.json

aws s3 cp processors.json s3://central-supelec-data-group4/input_data/processors.json
```
...
zip ods_soea.zip lambda_function.py ods.py soea.py utils/*
...
aws lambda invoke \
    --function-name ordonnanceur_group4 \
    --payload '{"input_bucket": "central-supelec-data-group4", "input_graph_key": "input_data/task_graph.json", "input_processors_key": "input_data/processors.json"}' \
...
```bash
aws s3 cp s3://central-supelec-data-group4/input_data/task_graph_results.json ordering.json
cat results.json
```

## Generating Plots Locally

Once you have `results.json`, you can generate the Gantt chart and DAG graph locally:

```bash
python generate_plots.py results.json task_graph.json <num_processors>
```

Note: it is not recommended to generate the graphs when you have more than 20 tasks, as they will not be very descriptive.
